...
public class FSEditLog implements NNStorageListener {
  ...
  /**
   * @return the first transaction ID in the current log segent
   * TODO: clarify when this changes and add a state check
   */
  ...
  private void checkAndRestoreFaultyJournals() {
    for (Iterator<JournalManager> iter = faultyJournals.iterator();
        iter.hasNext();) {
      JournalManager jm = iter.next();
      if (jm.canRestore()) {
        LOG.info("Restoring previously failed journal " + jm);
        iter.remove();
        journals.add(jm);
      } else {
        LOG.info("Previously failed journal " + jm + " is still unavailable.");
      }
    }
  }
  ...
  synchronized void startLogSegment(final long txId) {
    ...
    checkAndRestoreFaultyJournals();
    
    mapJournalsAndReportErrors(new JournalManagerClosure() {
      @Override
      public void apply(JournalManager jm) throws IOException {
        jm.startLogSegment(txId);
      }
  ...
  synchronized void endCurrentLogSegment() {
    ...
    mapJournalsAndReportErrors(new JournalManagerClosure() {
      @Override
      public void apply(JournalManager jm) throws IOException {
        jm.endLogSegment(curSegmentTxId, lastTxId);
      }
    }, "ending log segment");
